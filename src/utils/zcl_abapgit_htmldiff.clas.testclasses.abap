...
CLASS lcl_helper IMPLEMENTATION.
  ...
  METHOD htmldiff.
    ...
    DATA li_htmldiff TYPE REF TO zif_abapgit_htmldiff.

    li_htmldiff = zcl_abapgit_htmldiff=>create(
...
CLASS ltcl_htmldiff_test_1 IMPLEMENTATION.
  ...
  METHOD change_inside_tag.
    ...
    lv_exp = 'a <i class="icon"><i class="text"></i> c'.
...
CLASS zcl_abapgit_htmldiff DEFINITION LOCAL FRIENDS ltcl_calculate_operations.
...
CLASS ltcl_calculate_operations DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA mo_htmldiff TYPE REF TO zcl_abapgit_htmldiff.
    ...
    METHODS:
      ...
      calculate_operations
        IMPORTING
          ...
          !iv_count  TYPE i
          !iv_index  TYPE i
          !is_exp    TYPE zcl_abapgit_htmldiff=>ty_operation,
...
CLASS ltcl_calculate_operations IMPLEMENTATION.
  ...
  METHOD calculate_operations.
    ...
    DATA:
      lt_before TYPE zcl_abapgit_htmldiff=>ty_tokens,
      lt_after  TYPE zcl_abapgit_htmldiff=>ty_tokens,
    ...
    lt_ops = mo_htmldiff->calculate_operations(
      it_before_tokens = lt_before
  ...
  METHOD action_middle.
    ...
    DATA ls_exp TYPE zcl_abapgit_htmldiff=>ty_operation.
    ...
    calculate_operations(
      iv_before = 'this is a lot of text'
    ...
    calculate_operations(
      iv_before = 'this is what it sounds like'
  ...
  METHOD action_end.
    ...
    calculate_operations(
      iv_before = 'this is a dog that barks'
  ...
  METHOD calculate_simple.
    ...
    lt_ops = mo_htmldiff->calculate_simple(
      iv_before = ''
      iv_after  = '' ).

    cl_abap_unit_assert=>assert_initial( lt_ops ).
    ...
    READ TABLE lt_ops INTO ls_op INDEX 1.
    cl_abap_unit_assert=>assert_subrc( ).

    cl_abap_unit_assert=>assert_equals(
      act = ls_op-action
      exp = zcl_abapgit_htmldiff=>c_action-none ).
    ...
    cl_abap_unit_assert=>assert_equals(
      act = ls_op-action
      exp = zcl_abapgit_htmldiff=>c_action-delete ).
    ...
    cl_abap_unit_assert=>assert_equals(
      act = ls_op-action
      exp = zcl_abapgit_htmldiff=>c_action-insert ).
...
CLASS ltcl_diff DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA mi_htmldiff TYPE REF TO zif_abapgit_htmldiff.
...
CLASS ltcl_diff IMPLEMENTATION.
  ...
  METHOD setup.
    CREATE OBJECT mi_htmldiff TYPE zcl_abapgit_htmldiff.
  ENDMETHOD.
  ...
  METHOD test.
    ...
    lv_act = mi_htmldiff->htmldiff( iv_before = 'this is what I had'
...
CLASS zcl_abapgit_htmldiff DEFINITION LOCAL FRIENDS ltcl_find_matching_blocks.
    ...
    METHODS:
      ...
      find_match
        IMPORTING
          ...
          !iv_after        TYPE string
        RETURNING
          VALUE(rs_result) TYPE zcl_abapgit_htmldiff=>ty_match,
      ...
      find_matching_blocks
        IMPORTING
          ...
          !iv_after        TYPE string
        RETURNING
          VALUE(rt_result) TYPE zcl_abapgit_htmldiff=>ty_matches,
...
CLASS ltcl_find_matching_blocks IMPLEMENTATION.
  ...
  METHOD index_tokens.
    ...
    DATA:
      lt_find_these TYPE zcl_abapgit_htmldiff=>ty_tokens,
      lt_in_these   TYPE zcl_abapgit_htmldiff=>ty_tokens,
      ls_index      TYPE zcl_abapgit_htmldiff=>ty_index_row,
      lt_index      TYPE zcl_abapgit_htmldiff=>ty_index_tab,
      lv_loc        TYPE i.
    ...
    lt_index = mo_htmldiff->create_index(
      it_find_these = lt_find_these
  ...
  METHOD find_match.
    ...
    lt_index = mo_htmldiff->create_index(
      it_find_these = lt_before
      it_in_these   = lt_after ).

    rs_result = mo_htmldiff->find_match(
      it_before_tokens         = lt_before
      it_after_tokens          = lt_after
  ...
  METHOD find_match_1.
    ...
    DATA:
      ls_match TYPE zcl_abapgit_htmldiff=>ty_match,
      ls_exp   TYPE zcl_abapgit_htmldiff=>ty_match.

    " When there is a match, should match the match
    ls_match = find_match(
      iv_before = 'a dog bites'
  ...
  METHOD find_matching_blocks.
    ...
    rt_result = mo_htmldiff->find_matching_blocks(
      it_before_tokens = lt_before
      it_after_tokens  = lt_after ).
  ...
  METHOD find_matching_blocks_1.
    ...
    DATA lt_matches TYPE zcl_abapgit_htmldiff=>ty_matches.

    " When called with a single match, should return a match
    lt_matches = find_matching_blocks(
      iv_before = 'a dog bites'
  ...
  METHOD find_matching_blocks_2.
    ...
    DATA:
      ls_exp     TYPE zcl_abapgit_htmldiff=>ty_match,
      ls_match   TYPE zcl_abapgit_htmldiff=>ty_match,
      lt_matches TYPE zcl_abapgit_htmldiff=>ty_matches.

    " When called with multiple matches, should return 3 matches
    lt_matches = find_matching_blocks(
      iv_before = 'the dog bit a man'
...
CLASS zcl_abapgit_htmldiff DEFINITION LOCAL FRIENDS ltcl_html_to_token.
...
CLASS ltcl_html_to_token IMPLEMENTATION.
  ...
  METHOD test.
    ...
    DATA:
      lv_token  TYPE zcl_abapgit_htmldiff=>ty_token,
      lt_tokens TYPE zcl_abapgit_htmldiff=>ty_tokens,
      lt_exp    TYPE zcl_abapgit_htmldiff=>ty_tokens.
...
CLASS zcl_abapgit_htmldiff DEFINITION LOCAL FRIENDS ltcl_render_operations.
...
CLASS ltcl_render_operations IMPLEMENTATION.
  ...
  METHOD render_operations.
    ...
    rv_result = mo_htmldiff->render_operations(
      it_before_tokens = lt_before
      it_after_tokens  = lt_after
  ...
  METHOD test_equal.
    ...
    lv_act = render_operations(
      iv_before = 'this _ is _ a _ test'
  ...
  METHOD test_delete.
    ...
    lv_act = render_operations(
     iv_before = 'this _ is _ a _ test _ of _ stuff'
     iv_after  = 'this _ is _ a _ test' ).
  ...
  METHOD test_replace.
    ...
    lv_act = render_operations(
      iv_before = 'this _ is _ a _ break'
      iv_after  = 'this _ is _ a _ test' ).
  ...
  METHOD change_at_beginning_of_tag.
    ...
    lv_act = render_operations(
      iv_before = '<p> this _ is _ awesome </p>'
