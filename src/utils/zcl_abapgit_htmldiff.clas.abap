CLASS zcl_abapgit_htmldiff DEFINITION
  PUBLIC
  CREATE PUBLIC.
...
************************************************************************
  ...
  PUBLIC SECTION.
    ...
    INTERFACES zif_abapgit_htmldiff.
    ...
    CLASS-METHODS create
      IMPORTING
        ...
        !iv_support_chinese TYPE abap_bool DEFAULT abap_false
      RETURNING
        VALUE(ri_result)    TYPE REF TO zif_abapgit_htmldiff.
  ...
  PROTECTED SECTION.
    ...
    METHODS recurs_find_matching_blocks
      ...
      CHANGING
        ...
        !ct_result                TYPE ty_matches OPTIONAL.
...
CLASS zcl_abapgit_htmldiff IMPLEMENTATION.
  ...
  METHOD calculate_operations.
    ...
    lv_position_in_before = 0.
    lv_position_in_after  = 0.
    ...
    LOOP AT lt_matches INTO ls_match.
      ...
      IF lv_position_in_before = ls_match-start_in_before.
        ...
        ENDIF.
      ELSEIF lv_position_in_after = ls_match-start_in_after.
    ...
    LOOP AT lt_operations INTO ls_op.
      lv_whitespace = is_single_whitespace(
        is_op     = ls_op
  ...
  METHOD calculate_simple.
    ...
    IF iv_before IS INITIAL AND iv_after IS INITIAL.
      lv_action = ''.
    ELSEIF iv_before = iv_after.
      lv_action = c_action-none.
  ...
  METHOD create.
    ...
    CREATE OBJECT ri_result TYPE zcl_abapgit_htmldiff
      EXPORTING
  ...
  METHOD create_index.
    ...
    LOOP AT it_find_these ASSIGNING <lv_token>.
      ...
      READ TABLE rt_result ASSIGNING <ls_index> WITH TABLE KEY token = <lv_token>.
      IF sy-subrc <> 0.
        CLEAR ls_index.
  ...
  METHOD find_match.
    ...
    DO.
      IF iv_start_in_before <= iv_end_in_before.
        IF lv_index_in_before >= iv_end_in_before.
          EXIT.
        ENDIF.
      ELSEIF lv_index_in_before <= iv_end_in_before.
        EXIT.
      ENDIF.
    ...
    IF lv_best_match_length <> 0.
      ls_match = _new_match(
        iv_start_in_before = lv_best_match_in_before
  ...
  METHOD find_matching_blocks.
    ...
    lt_index_of_before_in_after = create_index(
      it_find_these = it_before_tokens
    ...
    CASE is_op-action.
      ...
      WHEN c_action-replace.
        ...
        rv_result = operation(
          is_op            = ls_op_del
          it_before_tokens = it_before_tokens
          it_after_tokens  = it_after_tokens ) && operation(
          is_op            = ls_op_ins
          it_before_tokens = it_before_tokens
          it_after_tokens  = it_after_tokens ).
  ...
  METHOD wrap.
    ...
    DO.
      ...
      lt_non_tags = consecutive_where(
        iv_start   = lv_position
  ...
  METHOD zif_abapgit_htmldiff~htmldiff.
    ...
    ELSE.
      ...
      lt_ops = calculate_operations(
        it_before_tokens = lt_before_tokens
        it_after_tokens  = lt_after_tokens ).

      rv_result = render_operations(
        it_before_tokens = lt_before_tokens
        it_after_tokens  = lt_after_tokens
