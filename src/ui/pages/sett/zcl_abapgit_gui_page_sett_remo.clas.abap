...
CLASS zcl_abapgit_gui_page_sett_remo IMPLEMENTATION.
      ...
      IF lv_popup_cancelled = abap_false.
        ...
        IF ls_pull IS NOT INITIAL.
          mo_form_data->set(
            ...
            iv_val = ls_pull-head_url && zif_abapgit_repo_online=>c_separator-pull_request && ls_pull-head_branch ).
        ENDIF.
      ENDIF.
  ...
  METHOD save_settings.
    ...
    IF mi_repo->is_offline( ) = abap_false AND
      ls_settings_new-head_type <> zif_abapgit_git_definitions=>c_head_types-pull_request AND
      ls_settings_new-head_type <> zif_abapgit_git_definitions=>c_head_types-fork.
  ...
  METHOD switch_to_pull_req.
    ...
    IF iv_revert = abap_false.
      SPLIT iv_pull AT zif_abapgit_repo_online=>c_separator-pull_request INTO lv_url lv_branch.
      li_repo_online->switch_origin(
        iv_url       = lv_url
        iv_branch    = zif_abapgit_git_definitions=>c_git_branch-heads_prefix && lv_branch
        iv_separator = zif_abapgit_repo_online=>c_separator-pull_request ).
    ENDIF.
    ...
    IF lv_offline = abap_false.
      ...
      CASE lv_head_type.
        ...
        WHEN zif_abapgit_git_definitions=>c_head_types-pull_request.
          ...
          IF lv_pull_request NA zif_abapgit_repo_online=>c_separator-pull_request.
            ro_validation_log->set(
              iv_key = c_id-pull_request
              iv_val = |Invalid format: must be <url>{ zif_abapgit_repo_online=>c_separator-pull_request }<branch>| ).
          ENDIF.
          SPLIT lv_pull_request AT zif_abapgit_repo_online=>c_separator-pull_request INTO lv_url lv_branch.
